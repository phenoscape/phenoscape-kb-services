...
object TaxaWithPhenotype {
  ...
  private val PhenotypeOfSome =
    NamedRestrictionGenerator.getClassRelationIRI(phenotype_of.getIRI)
  private val PartOfSome =
    NamedRestrictionGenerator.getClassRelationIRI(part_of.getIRI)
  private val HasPartSome =
    NamedRestrictionGenerator.getClassRelationIRI(has_part.getIRI)
  ...
  private def constructWhereClause(
    ...
    val homologyQueryPartsFut
      ...
      val homSubquery =
        phenotypeSubQueryFor(Option(otherEntity), quality, phenotypeOpt, false)
      ...
      if (includeParts) {
        val homPartsSubquery =
          phenotypeSubQueryFor(Option(otherEntity), quality, phenotypeOpt, true)
    ...
    for {
      ...
      val basicSubquery =
        phenotypeSubQueryFor(entity, quality, phenotypeOpt, false)
      val basic =
      ...
      if (includeParts) {
        val partsSubquery =
          phenotypeSubQueryFor(entity, quality, phenotypeOpt, true)
        val parts =
  ...
  def phenotypeSubQueryFor(
      entity: Option[IRI],
      quality: QualitySpec,
      phenotypeOpt: Option[IRI],
      parts: Boolean
  ): Set[BlazegraphNamedSubquery] = quality match {
    case PhenotypicQuality(qualityOpt) =>
      phenotypicQualitySubQueryFor(entity, qualityOpt, phenotypeOpt, parts)
    case InferredPresence =>
      entity
        .map(e => entityPhenotypeSubQueryFor(e, parts, implies_presence_of_some)
        )
        .toSet
    case InferredAbsence =>
      entity.map(e => entityPhenotypeSubQueryFor(e, parts, ABSENCE_OF)).toSet
  }

  def phenotypicQualitySubQueryFor(
      entity: Option[IRI],
      quality: Option[IRI],
      phenotype: Option[IRI],
      parts: Boolean
  ): Set[BlazegraphNamedSubquery] = {
    val entityPattern =
      entity.map(entityPhenotypeSubQueryFor(_, parts, PhenotypeOfSome))
  ...
  def entityPhenotypeSubQueryFor(
      entity: IRI,
      parts: Boolean,
      relation: IRI
