...
@Path("annotation/inheres_in_part_of")
class NearestInheresInPartOf(@QueryParam("annotation") var annotationIDParam: String) {
  ...
  val inheresInPartOfSome = NamedRestrictionGenerator.getClassRelationIRI(Vocab.inheres_in_part_of.getIRI)
  ...
  def superTypeQuery(annotationID: String): Query = {
    ...
    select_distinct('description) from "http://kb.phenoscape.org/" where (
      bgp(
        ...
        t('description, inheresInPartOfSome, 'bearer),
