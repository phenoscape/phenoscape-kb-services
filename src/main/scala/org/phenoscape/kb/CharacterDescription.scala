...
object CharacterDescription {
  ...
  def queryVariationProfile(
    ...
    App.executeSPARQLQuery(
      ...
      CharacterDescriptionAnnotation(_)
    )

  def queryVariationProfileTotal(taxa: Seq[IRI]): Future[Int] =
    App
      .executeSPARQLQuery(buildVariationProfileTotalQuery(taxa))
      .map(ResultCount.count)

  def withIRI(iri: IRI): Future[Option[CharacterDescription]] =
    App
      .executeSPARQLQuery(
        buildCharacterDescriptionQuery(iri),
        fromQuerySolution(iri)
      )
      .map(_.headOption)
  ...
  def buildBasicQuery(
    ...
    val entityPatterns =
      if (entity == owlThing) Nil
        ...
        t('phenotype, ps_entity_term | ps_related_entity_term, 'entity) :: t(
          'entity,
          rdfsSubClassOf,
          entity.asOMN
        ) :: Nil
      ...
      if (publications.isEmpty) Nil
        ...
        new ElementFilter(
          new E_OneOf(
            new ExprVar('matrix),
            new ExprList(
              publications.map(new NodeValueNode(_)).toBuffer[Expr].asJava
            )
          )
        ) :: Nil
  ...
  def buildVariationProfileTotalQuery(taxa: Seq[IRI]): Query = {
    ...
    query.getProject.add(
      Var.alloc("count"),
      query.allocAggregate(new AggCountVarDistinct(new ExprVar("phenotype")))
    )
    query
  }
    ...
    val filters =
      if (taxa.isEmpty) Nil
        ...
        new ElementFilter(
          new E_OneOf(
            new ExprVar('taxon),
            new ExprList(taxa.map(new NodeValueNode(_)).toBuffer[Expr].asJava)
          )
        ) :: Nil
    select_distinct() from "http://kb.phenoscape.org/" where (bgp(
...
object CharacterDescriptionAnnotation {
  ...
  def apply(result: QuerySolution): CharacterDescriptionAnnotation =
    CharacterDescriptionAnnotation(
      IRI.create(result.getResource("state").getURI),
      result.getLiteral("state_desc").getLexicalForm,
