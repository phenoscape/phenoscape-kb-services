    ...
    TermDetails.computedLabel(IRI.create(result.getResource("phenotype").getURI)).map { phenotype =>
      AnnotatedCharacterDescription(
        CharacterDescription(
          ...
          CharacterMatrix(
            IRI.create(result.getResource("matrix").getURI),
            result.getLiteral("matrix_label").getLexicalForm), MinimalTerm(IRI.create(result.getResource("character").getURI), Option(result.getLiteral("character_label").getLexicalForm))),
        phenotype)
    }
  }
