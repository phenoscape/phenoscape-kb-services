...
object Gene {
  ...
  private val geneIDPrefixToTaxon = Map(
    ...
    "http://www.informatics.jax.org" -> Taxon(
  ...
  def withIRI(iri: IRI): Future[Option[Gene]] =
    App
      .executeSPARQLQuery(buildGeneQuery(iri), Gene.fromIRIQuery(iri))
      .map(_.headOption)
  ...
  def buildTotalQuery(
    ...
    query.getProject.add(
      Var.alloc("count"),
      query.allocAggregate(new AggCountVarDistinct(new ExprVar("gene")))
    )
    query
  }
  ...
  implicit val GenesTextMarshaller: ToEntityMarshaller[Seq[Gene]] = Marshaller
