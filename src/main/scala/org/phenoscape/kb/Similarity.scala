...
object Similarity {
    ...
    val labelled = unlabelleds.foldLeft(
      ...
      (for {
        ...
        val queryAnnotationLabelFuture = labelMap
          ...
          .getOrElse(
            Term
              .computedLabel(pair.queryAnnotation)
              .map(_.label.getOrElse(pair.queryAnnotation.toString))
          )
        val corpusAnnotationLabelFuture = labelMap
          ...
          .getOrElse(
            Term
              .computedLabel(pair.corpusAnnotation)
  ...
  def profileSize(profileSubject: IRI): Future[Int] = {
    ...
    query.getProject.add(
      Var.alloc("count"),
      query.allocAggregate(new AggCountVarDistinct(new ExprVar("annotation")))
    )
  ...
  def corpusSize(corpusGraph: IRI): Future[Int] = {
    val query = select() from corpusGraph.toString where (bgp(
      t('comparison, for_corpus_profile, 'profile)
    ))
    query.getProject.add(
      Var.alloc("count"),
      query.allocAggregate(new AggCountVarDistinct(new ExprVar("profile")))
    )
    App.executeSPARQLQuery(query).map(ResultCount.count)
  }
  ...
  def similarityProfileQuery(
    ...
    val query = select_distinct(
      ...
      'median_score,
      'expect_score
    ) from "http://kb.phenoscape.org/" from corpusGraph.toString where (
      bgp(
  ...
  def subsumedAnnotationsQuery(
    ...
    new ElementSubQuery(
      select('annotation) where (new ElementNamedGraph(
  ...
  def constructMatchFor(queryItem: IRI): QuerySolution => SimilarityMatch =
    (result: QuerySolution) =>
      SimilarityMatch(
        MinimalTerm(
          IRI.create(result.getResource("corpus_item").getURI),
  ...
  private def classSubsumers(iri: IRI): Future[Set[IRI]] = {
    ...
    App
      .executeSPARQLQueryString(
        query.text,
        qs => IRI.create(qs.getResource("subsumer").getURI)
      )
      .map(_.toSet)
  }
...
object Subsumer {
  ...
  def fromQuery(result: QuerySolution): Future[Subsumer] = {
    ...
    Term
      .computedLabel(iri)
      .map(term => Subsumer(term, result.getLiteral("ic").getDouble))
  }
