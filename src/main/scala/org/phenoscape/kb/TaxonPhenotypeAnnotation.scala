...
object TaxonPhenotypeAnnotation {
  ...
  def queryAnnotations(entity: Option[IRI],
                       quality: QualitySpec,
  ...
  def queryAnnotationsStream(entity: Option[IRI],
                             quality: QualitySpec,
                             inTaxonOpt: Option[IRI],
                             phenotypeOpt: Option[IRI],
                             publicationOpt: Option[IRI],
                             ...
                             includeSerialHomologs: Boolean,
                             limit: Int = 20,
                             offset: Int = 0): Source[TaxonPhenotypeAnnotation, NotUsed] = {
    val futQuery = TaxonAnnotations.buildQuery(entity,
                                               quality,
                                               inTaxonOpt,
                                               phenotypeOpt,
                                               publicationOpt,
  ...
  def queryAnnotationsTotal(entity: Option[IRI],
                            quality: QualitySpec,
                            inTaxonOpt: Option[IRI],
                            phenotypeOpt: Option[IRI],
                            publicationOpt: Option[IRI],
  ...
  def fromQueryResult(result: QuerySolution): TaxonPhenotypeAnnotation =
  ...
  def facetTaxonAnnotationsByEntity(focalEntity: Option[IRI],
                                    quality: QualitySpec,
                                    inTaxonOpt: Option[IRI],
                                    publicationOpt: Option[IRI],
                                    includeParts: Boolean,
  ...
  def facetTaxonAnnotationsByQuality(focalQuality: Option[IRI],
                                     entity: Option[IRI],
                                     inTaxonOpt: Option[IRI],
                                     publicationOpt: Option[IRI],
                                     includeParts: Boolean,
  ...
  def facetTaxonAnnotationsByTaxon(focalTaxon: Option[IRI],
                                   entity: Option[IRI],
                                   quality: QualitySpec,
                                   publicationOpt: Option[IRI],
                                   includeParts: Boolean,
  ...
  def annotationSources(taxon: IRI, phenotype: IRI): Future[Seq[AnnotationSource]] = {
    ...
    App.executeSPARQLQueryString(
      ...
      res =>
        AnnotationSource(
          MinimalTerm(IRI.create(res.getResource("pub").getURI), Some(res.getLiteral("pub_label").getLexicalForm)),
          res.getLiteral("char_num").getInt,
  ...
  val AnnotationTSVMarshaller: ToEntityMarshaller[TaxonPhenotypeAnnotation] =
    Marshaller.stringMarshaller(MediaTypes.`text/tab-separated-values`).compose { annotation =>
      annotation.toString
    }

  val AnnotationsTextMarshaller: ToEntityMarshaller[Seq[TaxonPhenotypeAnnotation]] =
  ...
  implicit val ComboTaxonPhenotypeAnnotationsMarshaller =
    Marshaller.oneOf(AnnotationsTextMarshaller, JSONResultItemsMarshaller)

}

final case class AnnotationSource(publication: MinimalTerm, characterNum: Int, character: String, state: String)
