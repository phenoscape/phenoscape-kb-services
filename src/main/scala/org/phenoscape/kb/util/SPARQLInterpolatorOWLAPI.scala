...
object SPARQLInterpolatorOWLAPI {
  ...
  implicit val embedOWLObjectPropertyInSPARQL =
    SPARQLInterpolator.embed[OWLObjectProperty](Case(SPARQLContext, SPARQLContext) { obj =>
      val pss = new ParameterizedSparqlString()
      pss.appendIri(obj.getIRI.toString)
      pss.toString
    })

  implicit val embedOWLAnnotationPropertyInSPARQL =
    SPARQLInterpolator.embed[OWLAnnotationProperty](Case(SPARQLContext, SPARQLContext) { obj =>
      val pss = new ParameterizedSparqlString()
      pss.appendIri(obj.getIRI.toString)
      pss.toString
    })
