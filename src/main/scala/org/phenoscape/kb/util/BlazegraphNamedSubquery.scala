...
object BlazegraphNamedSubquery {
  ...
  def apply(queryText: QueryText): BlazegraphNamedSubquery =
    BlazegraphNamedSubquery(queryText, Set(UUID.randomUUID.toString.replaceAllLiterally("-", "")))

  def unifyQueries(queries: Set[BlazegraphNamedSubquery]): Set[BlazegraphNamedSubquery] =
    if (queries.nonEmpty)
      queries
        .groupBy(_.queryText)
        .values
        .map(_.reduce((a, b) => BlazegraphNamedSubquery(a.queryText, a.ids ++ b.ids)))
        .toSet
    else queries
