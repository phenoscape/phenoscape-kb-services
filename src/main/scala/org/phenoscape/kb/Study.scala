...
object Study {
  ...
  def annotatedTaxaTotal(study: IRI): Future[Int] =
    App
      .executeSPARQLQuery(buildAnnotatedTaxaTotalQuery(study))
  ...
  def annotatedPhenotypes(
    ...
    val futureDescriptions = Term.label(study).map { studyTermOpt =>
      studyTermOpt.map { studyTerm =>
        val matrix = CharacterMatrix(
          studyTerm.iri,
          studyTerm.label.getOrElse(studyTerm.iri.toString)
        )
        App.executeSPARQLQuery(
          buildPhenotypesQuery(study, limit, offset),
          annotationFromQueryResult(matrix)
        )
      }
    }
    for {
  ...
  def annotatedPhenotypesTotal(study: IRI): Future[Int] =
    App
      .executeSPARQLQuery(buildPhenotypesTotalQuery(study))
      .map(ResultCount.count)

  private def buildAnnotatedTaxaSubQuery(study: IRI): Query =
    select_distinct('taxon, 'taxon_label) where (bgp(
  ...
  def buildAnnotatedTaxaTotalQuery(study: IRI): Query = {
    val query = select() from "http://kb.phenoscape.org/" where (new ElementSubQuery(
      buildAnnotatedTaxaSubQuery(study)
    ))
    query.getProject
  ...
  def matrix(study: IRI, model: Model): Elem = { //FIXME add about=#
    ...
    def idForStateGroup(states: Set[String]): String =
      if (states.size == 1) s"state_${UUID.randomUUID.toString}"
        ...
        states
          .map(state => idForStateGroup(Set(state)))
          .toSeq
          .sorted
          .mkString("_")
    ...
    def label(uri: String): String =
      model
        .getProperty(ResourceFactory.createResource(uri), rdfsLabel)
        .getString
    ...
    <nexml xmlns="http://www.nexml.org/2009" xmlns:dc="http://purl.org/dc/terms/" xmlns:dwc="http://rs.tdwg.org/dwc/terms/" xmlns:obo="http://purl.obolibrary.org/obo/" xmlns:ps="http://purl.org/phenoscape/vocab.owl#" xmlns:rdfs="http://www.w3.org/2000/01/rdf-schema#" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" version="0.9" xsi:schemaLocation="http://www.nexml.org/2009 http://www.nexml.org/2009/nexml.xsd http://www.bioontologies.org/obd/schema/pheno http://purl.org/phenoscape/phenoxml.xsd">
      ...
      <meta xsi:type="LiteralMeta" property="dc:bibliographicCitation">
        ...
        model
          .getProperty(
            ResourceFactory.createResource(study.toString),
            ResourceFactory
              .createProperty(dcBibliographicCitation.getIRI.toString)
          )
          .getString
      }
      </meta>
      <meta xsi:type="LiteralMeta" property="rdfs:label" content={
        label(study.toString)
      }/>
    </meta>
    }<otus id={otusID}>
      {
      ...
      <characters id={s"characters_${UUID.randomUUID.toString}"} xsi:type="StandardCells" otus={
      otusID
    }>
        <format>
          {
          ...
          if (stateGroup.size < 2)
            <state id={
              ...
              val phenotypeURI = model
                .getProperty(
                  ResourceFactory.createResource(stateGroup.head),
                  describes_phenotype
                )
                .getResource
                .getURI
              ...
              <meta xsi:type="ResourceMeta" rel="ps:describes_phenotype" href={
                phenotypeURI
              }/>
            }
              <meta xsi:type="ResourceMeta" rel="obo:IAO_0000219" href={
              stateGroup.head
            }/>
            </state>
          else
            <polymorphic_state_set id={
              stateGroupID
            } symbol={
              symbol(stateGroup)
            }>
      ...
      for {
        ...
        for {
          ...
          stateGroupIDs(
            model
              .listObjectsOfProperty(cell, has_state)
              .asScala
              .map(_.asResource.getURI)
              .toSet
          )
  ...
  private def queryToTerm(result: QuerySolution): MinimalTerm =
    MinimalTerm(
      ...
      Some(result.getLiteral("matrix_label").getLexicalForm)
    )
